...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public void addItem(OrderId orderId, OrderTicketForm orderTicketForm) throws OrderIdNotExistException {
        Order order = orderRepository.findById(orderId).orElseThrow(OrderIdNotExistException::new);
        order.addItem(orderTicketForm.getConcertTicket(), orderTicketForm.getQuantity());
        orderRepository.saveAndFlush(order);
    }

    @Override
    public void deleteItem(OrderId orderId, OrderTicketId orderTicketId) throws OrderIdNotExistException, OrderTicketIdNotExistException {
        Order order = orderRepository.findById(orderId).orElseThrow(OrderIdNotExistException::new);
    ...
    private Order toDomainObject(OrderForm orderForm) {
        ...
        orderForm.getItems().forEach(item -> order.addItem(item.getConcertTicket(), item.getQuantity()));
        return order;
    }
}
